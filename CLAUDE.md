...
| `typescript` reste en 6 | **Notre code est prêt** : TS 7 typecheck les quatre workspaces sans une erreur. C'est `@typescript-eslint@8.69.0` qui plante dessus (`Cannot read properties of undefined (reading 'Intrinsic')`), et il vient d'`eslint-config-expo` | Une ligne à changer le jour où `@typescript-eslint` supporte TS 7. Le dernier obstacle côté projet — `moduleResolution: node10` — est déjà levé |
